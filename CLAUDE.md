...
23. Phone: backend code-complete (agent_calls, call-webhook, Calls page, usage — verified 17/17, commit `0ad59bc`). **Remaining: add `VAPI_WEBHOOK_SECRET` to Vercel** (credentials-only gap — do on final integration day per Hard Rule 20). Separately: Oussama connects Twilio number → wire inbound to Vapi agent.
...
- **Phone agent backend: code-complete.** `agent_calls` table, RLS, indexes, `call-webhook` route logic, Calls page retrieval, context stats, voice-minute usage — all verified end-to-end (17/17, commit `0ad59bc`). **External blocker only: `VAPI_WEBHOOK_SECRET` is a placeholder env var** (not yet in Vercel — see §8 Pending). Real Vapi end-of-call payloads won't reach the webhook until this credential is set; that is a credentials gap, not a code gap. Do not revisit until final integration day (Hard Rule 20).
