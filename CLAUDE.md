...
12. Build real plan enforcement: usage tracking per tenant (messages, voice minutes), server-side limit checks on every API route, clean upgrade prompts when at cap. **5-step build (Step 1 code-complete — commit `1ddc730`, pending Oussama running `migration_v11.sql`).**
    - Step 1 ✅ (code-complete, SQL pending) — `supabase/migration_v11.sql`: adds `tenant_id UUID` to `messages`, backfills from conversations join, adds `idx_messages_tenant_period` partial index. **⚠️ RUN IN SUPABASE SQL EDITOR:** `supabase/migration_v11.sql`.
    - Step 2 — Instrument writes: add `tenant_id` to both `.insert()` calls in `ai/reply/route.ts` (after migration_v11 runs).
    - Step 3 — Usage helper (`src/lib/usage.ts`) + enforcement check in `ai/reply` (messages cap) and `call-webhook` (voice minutes cap). Returns 429 `{ error, limitType, used, limit }` when over.
    - Step 4 — New `/api/stats/usage` GET route: returns `{ messages: { used, limit }, voiceMinutes: { used, limit }, periodStart }` for dashboard.
    - Step 5 — Dashboard usage meters + upgrade prompt UI (same UpgradeModal pattern as channels page).
