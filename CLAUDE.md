...
12. Build real plan enforcement: usage tracking per tenant (messages, voice minutes), server-side limit checks on every API route, clean upgrade prompts when at cap. **5-step build (Step 2 done — commit `0ef2914`).**
    ...
    - Step 3 — Usage helper (`src/lib/usage.ts`) + enforcement check in `ai/reply` (messages cap) and `call-webhook` (voice minutes cap). Returns 429 `{ error, limitType, used, limit }` when over. **NEXT.**
