...
- **Phase 3 — Design system formalization (DONE, commits `2b1cd03`, `0c94db9`):**
  ...
  (5) **FIX 4 (enforcement) — partial enforcement, one gap found**: Fonts, mood, radii, and font-sizes are fully server-enforced. **Gap**: `coerceDesignDNA` in `route.ts` (line 582–585) accepts any valid 6-char hex for `bg`/`text`/`muted` — GPT can override the mood's default palette colors. `accent` is prompt-constrained to an approved list (Part 5 of `buildFillSystem`) but not server-validated against that list. In practice GPT echoes the mood defaults, so this is low risk, but not enforced. **Proposed fix** (not built — confirm before proceeding): in `coerceDesignDNA`, drop GPT's `bg`/`text`/`muted` entirely (always use `MOOD_DEFAULT_DNA[mood].palette`) and validate `accent` against a server-side `APPROVED_ACCENTS` set matching the prompt's Part 5 table, falling back to the mood default if not in set.
  ...
  **Verification**: `e2e-test-phase3.ts` — 62/62 checks (token defs, spacing wires, type wires, color/radius fixes, intentional exceptions, Phase 2e hamburger regression, multi-preset rendering). Phase 2e real-pipeline regression: 3/3 tests, 44/44 checks.
