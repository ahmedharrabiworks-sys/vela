...
- **Phase 3 — Design system formalization (DONE, commit `2b1cd03`):**
  ...
  (2) **Spacing scale tokens**: `--sp-xs:8px`, `--sp-sm:16px`, `--sp-md:24px`, `--sp-lg:32px`, `--sp-xl:48px`, `--sp-2xl:64px`, `--sp-3xl:96px` — defined in `:root`, available for future component mapping.
  ...
  (5) **FIX 4 (enforcement) — already in place**: GPT cannot output colors/fonts/radii by architecture (selects mood/strategy only; server resolves to `DesignTokens`). `_textStyles` field on `WebsiteSpec` is dead code — declared but never applied in renderer. No enforcement gap found.
  **Not completed in this phase**: shadows (no rogue box-shadow found outside intentional use), full component mapping to type/spacing tokens (conservative approach — tokens defined, selective mapping only to avoid regression). This is a foundation phase — tokens now exist and are correct; full sweep can happen incrementally.
