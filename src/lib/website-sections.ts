...
// ── Photo helper — renders img or gradient placeholder ────────────────────────
function photo(src: string | undefined, alt: string, cls: string, style = ""): string {
  if (src) {
    return `<img src="${esc(src)}" alt="${esc(alt)}" class="${cls}" style="${style}" loading="lazy">`;
  }
...
// ── Hero — full-bleed photograph, enormous type overlay ───────────────────────
export function renderHero(
  _t: DesignTokens,
  c: { eyebrow?: string; headline?: string; subheadline?: string; ctaPrimary?: string; ctaSecondary?: string },
  imageUrl?: string
): string {
  return `<section class="ws-hero" id="hero">
  ${imageUrl
    ? `<img src="${esc(imageUrl)}" alt="${esc(c.headline || "Hero")}" class="ws-hero-img" fetchpriority="high">`
    : `<div class="ws-hero-img" style="background:linear-gradient(135deg,var(--hero-bg) 0%,color-mix(in srgb,var(--hero-bg) 60%,var(--accent)) 100%);"></div>`
  }
  <div class="ws-hero-overlay"></div>
  <div class="ws-hero-content">
    ${c.eyebrow ? `<p class="ws-hero-eyebrow">${esc(c.eyebrow)}</p>` : ""}
    <h1 class="ws-hero-headline">${esc(c.headline || "Welcome")}</h1>
...
// ── Services — editorial numbered grid, no icon circles ───────────────────────
  ...
  <div class="ws-container">
    ...
    <div class="ws-services-grid">
      ...
      <div class="ws-service-card">
        <p class="ws-service-num">0${i + 1}</p>
...
// ── Testimonials — only renders when real quotes exist ────────────────────────
