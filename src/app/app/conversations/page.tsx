      ...
      <div className={`flex-col bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[#F3F4F6]">
          {filtered.map((conv) => {
            ...
            return (
              <button key={conv.id} onClick={() => { setSelected(conv); setShowThread(true); }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className="text-xs font-semibold text-[#111111] truncate">{conv.name}</span>
      ...
      <div className={`flex-col bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden
        ...
        <div className="flex items-center gap-3 px-4 md:px-5 py-4 border-b border-[#F3F4F6]">
          <button onClick={() => setShowThread(false)}
            className="md:hidden p-2 -ml-1 rounded-xl text-[#6B7280] hover:text-[#111111] hover:bg-[#F3F4F6] transition-all min-h-[44px] min-w-[44px] flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-[#111111] text-sm leading-tight">{selected.name}</p>
            <p className="text-[10px] text-[#6B7280] mt-0.5">via {selected.channel}</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="text-xs font-semibold px-3 py-1.5 rounded-lg text-white hover:opacity-90 transition-opacity min-h-[44px]" style={{ background: "#FF6B35" }}>
              + Book
            </button>
          </div>
        </div>
        ...
        <div className="px-4 md:px-5 py-4 border-t border-[#F3F4F6] bg-white">
          ...
          <div className="flex gap-2 items-end">
            <textarea value={reply} onChange={(e) => setReply(e.target.value)}
              ...
              className="flex-1 text-sm resize-none rounded-xl border border-[#E5E7EB] px-4 py-3 text-[#111111] placeholder:text-[#9CA3AF] focus:outline-none focus:border-[#FF6B35]/50 transition-colors min-h-[52px]" />
