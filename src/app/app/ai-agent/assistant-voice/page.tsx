  ...
  const save = async () => {
    ...
    try {
      await fetch("/api/ai-agent/assistant-settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ voiceId: selectedVoice, speed: clampSpeed(speed) }),
      });
      setSaved(true);
  ...
  return (
    <div style={{ background: bg, margin: "-20px -16px -32px", padding: "20px 16px 32px" }}>
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        {/* Header */}
        <div>
          <h1 className="text-xl font-bold mb-1" style={{ color: textPrimary }}>Assistant Voice</h1>
          <p className="text-sm" style={{ color: textMuted }}>
            Choose how Vela sounds when talking to you — saved separately from your phone agent&apos;s voice.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-3 rounded-2xl border p-5" style={{ background: cardBg, borderColor: border }}>
            <h2 className="text-sm font-semibold mb-4" style={{ color: textPrimary }}>Select a Voice</h2>
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            <div className="rounded-2xl border p-5 flex flex-col gap-3" style={{ background: cardBg, borderColor: border }}>
              <p className="text-xs" style={{ color: textMuted }}>
                Changes apply to your next &ldquo;Talk to Vela&rdquo; session — active calls are not affected.
              </p>
              {saved && (
