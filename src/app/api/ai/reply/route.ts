    ...
    try {
      ...
      const extract = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `Current datetime (ISO 8601): ${new Date().toISOString()}. Look at the customer's latest message, with recent conversation context, and determine if they are proposing ONE concrete, fully-resolved date AND time for a booking action -- a first-time booking, an explicit reschedule to a different time, or clearly accepting a specific NEW slot you (the AI) just freshly offered as an alternative (e.g. you offered "3:30 or 4:00" and they reply "4:00 works"). Resolve relative dates ("tomorrow", "next Tuesday", "the 15th at 3pm") using the current datetime above. IMPORTANT: this system has no real timezone conversion anywhere -- the current datetime given above already uses the exact same clock convention this business's appointments are stored and displayed in. Never apply any timezone shift, offset, or "helpful" conversion based on the business's city or country -- write the literal clock hour the customer said, unmodified, with a Z suffix (e.g. customer says "3pm" -> "...T15:00:00Z", never adjusted). Also identify which service (if any) is being discussed, matching one of these real services if possible: ${kbServices.map((s) => s.name).join(", ") || "(none configured)"}. Reply ONLY valid JSON: {"candidateDateTime": "ISO 8601 or null", "candidateService": "exact matching service name or null"}. Return null values if no concrete date+time is being proposed for a booking action right now. CRITICAL, DO NOT extract a date/time for a plain STATUS question about an appointment the customer already has -- e.g. "is my appointment still confirmed?", "do I still have my appointment", "what time was my appointment again?", "did my booking go through?" -- these are read-only questions about an EXISTING booking, not a new booking/reschedule/slot-acceptance, even when a specific date/time was mentioned earlier in the conversation and even though the word "confirm/confirmed" appears. Only extract when the customer is actually proposing or agreeing to a time for something to be booked or moved, never when they are just asking about the status of a booking that may already exist.`,
          },
